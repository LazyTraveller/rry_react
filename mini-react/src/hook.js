...
const Dispatcher = (() => {
  function mountWorkInProgressHook() { // mount 时调用
    ...
    if (!fiber.memoizedState) {
      ...
      workInProgressHook.next = hook // 不是第一个的话就加上一个hook的后面，形成链表
    }
  ...
  function updateWorkInProgressHook() { // update 时调用
    ...
    workInProgressHook = workInProgressHook.next // 一下一个hook
    return curHook
  }
  ...
  function useState(initialState) {
    ...
    let baseState = hook.memoizedState // 初始化状态
